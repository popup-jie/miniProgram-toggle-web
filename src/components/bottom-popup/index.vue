<template>
  <div class="components-class">
    <div  class=" custom-class"><van-popup :show="show" @close="onClose" position="bottom" custom-class="bottom-popup":custom-style="max-height: height%"><slot  name="title" v-if="useTitleSlot"><div  class="title" wx:else:style="titleStyle">
      {{title}}
      <img  src="/images/search_shut.png" class="closeIcon" @click="onClose()" /></div><slot ></slot><slot  name="footer" v-if="useFooterSlot"></slot><div  class="footer" wx:else><van-button  round custom-class="saveBtn" @click.stop="saveHandle()">
        {{comfirmText || '确认'}}
      </van-button></div></slot></van-popup></div>
  </div>
</template>
